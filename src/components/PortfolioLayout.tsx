...
export function PageShell({ active, children }: { active: NavPage; children: React.ReactNode }) {
  ...
  return (
    <div className="l-page-shell" lang={lang} style={{ position: 'relative', color: T.fg, fontFamily: T.serif, minHeight: '100%', fontSize: 15, lineHeight: 1.6 }}>
...
export function Row({
  ...
  return (
    <div className={`l-row${hasLeft ? '' : ' is-without-left'}`} style={{ padding: '24px 0', borderTop: `1px solid ${T.rule}` }}>
      {hasLeft && (
      ...
      <div>
        ...
        {body && <p style={{ color: T.fgDim, fontSize: 15, margin: 0, lineHeight: 1.6 }}>{body}</p>}
        {right}
      </div>
    </div>
  );
