        ...
        <div key={yg.year} style={{ marginTop: 24 }}>
          ...
          {yg.items.map((p, i) => (
            <div key={i} className="l-pub-row" style={{ padding: '22px 0', borderTop: `1px solid ${T.rule}` }}>
              ...
              <div>
                <h4 style={{ fontFamily: T.serif, fontSize: 17, fontWeight: 500, color: T.fg, margin: '0 0 6px', lineHeight: 1.3 }}>{p.title}</h4>
      ...
      {academicService.map((s, i) => (
        <div key={i} className="l-service-row" style={{ padding: '24px 0', borderTop: `1px solid ${T.rule}` }}>
          ...
          <div>
            ...
            {s.description && (
              <p style={{ color: T.fgDim, fontSize: 15, margin: 0, lineHeight: 1.6, maxWidth: 640 }}>{s.description}</p>
            )}
          </div>
        </div>
      ))}
